...
void main() {
  ...
  // La vague suit la souris : bosse radiale ondulante autour du curseur
  float md = distance(p, u_mouse);
  float sigma = u_res.y * 0.16;
  float infl = exp(-md * md / (2.0 * sigma * sigma));
  vec2 mdir = md > 0.5 ? (p - u_mouse) / md : vec2(0.0);
  disp += mdir * infl * 0.016 * sin(md * 0.045 - u_time * 3.2);
...
export default function SplashIntro() {
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      // La souris suit avec un léger retard (lerp) → la vague « traîne » derrière.
